...
| Predictability (actual/planned BV %) | For each committed Feature in `art/<art-slug>/pi-<pi-slug>/objectives.md`, read `planned_bv` and `actual_bv` (filled at I&A). Sum `actual_bv` / sum `planned_bv` × 100. | `art/<art-slug>/pi-<pi-slug>/objectives.md` |
